...
#targets <- read.table("~/Desktop/miRNA1304 targets.csv", quote="\"", comment.char="")
targets
targets1<-as.character(targets1)
list(targets=targets1)

#Gage Results(miRNA-1304 response) for greater than, less than, and stat geometric mean#
results<-gage(exprs = y, gsets = list(targets=targets1), samp = z, full.table = TRUE, saaPrep = gagePrep, saaTest = gs.tTest, same.dir = TRUE, test4up=TRUE)


#Essenial Genes from Target List#

essGene(targets1, exprs= y, ref = NULL, samp = z, use.fold = TRUE, rank.abs = FALSE, use.chi = FALSE, chi.p = 0.05)

#Attempt to get essential gene list although I do not understand how to get the p-value matrix from results to setp#
esset.grp(setp = , exprs=y, gsets = list(targets=targets1) , ref = NULL, samp = z, test4up = TRUE, same.dir = TRUE, use.fold = TRUE, cutoff = 0.05, use.q = FALSE, pc = 10^-10, output = TRUE, outname = "esset.grp", make.plot = TRUE, pdf.size = c(7, 7), core.counts = FALSE, get.essets = TRUE, bins = 10, bsize = 1, cex = 0.5, layoutType = "circo", name.str =c(10, 100))
